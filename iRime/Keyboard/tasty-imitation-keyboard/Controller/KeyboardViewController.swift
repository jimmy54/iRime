                ...
                for key in rowKeys {
                    if let keyView = self.layout?.viewForKey(key) {
                        ...
                        case Key.KeyType.Space:
                            keyView.addTarget(self, action: Selector("spaceDragInside:withEvent:"), forControlEvents: .TouchDragInside)
                            keyView.addTarget(self, action: Selector("spaceUpInside:withEvent:"), forControlEvents: .TouchUpInside)
                            keyView.addTarget(self, action: Selector("spaceDown:withEvent:"), forControlEvents: .TouchDown)
...
//        self.textDocumentProxy.deleteBackward()
    ...
    var spaceDragPoint:CGPoint = CGPointZero
    var spaceDragPointCount:NSInteger = 0
    var spaceDragIsMoving: Bool = false
    
    
    
    func spaceUpInside(sender: KeyboardKey, withEvent event:UIEvent) {
        
        spaceDragPoint = CGPointMake(0, 0)
    }
    func spaceDown(sender: KeyboardKey, withEvent event:UIEvent) {
        
        let touch = event.allTouches()?.first
        let p = touch?.locationInView(sender)
        
        spaceDragPoint = p!
    }
        
    func spaceDragInside(sender: KeyboardKey,  withEvent event:UIEvent) {
    ...
    func spaceTap(tap:UITapGestureRecognizer) {
        
    }
    
    
    func spaceSwip(swip: UISwipeGestureRecognizer) {
